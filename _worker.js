        ...
        if (!upgradeHeader || upgradeHeader !== 'websocket') {
            ...
            if (访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                ...
                if (url.searchParams.get('token') === 订阅TOKEN) {
                    ...
                    const UD = Math.floor(((now - today.getTime()) / 86400000) * 0.3333 * 1099511627776 / 2);
                    let pagesSum = UD, workersSum = UD, total = 99999 * 1099511627776;
                    ...
                    if (订阅类型 === 'mixed') {
                        ...
                        if (!url.searchParams.has('sub') && config_JSON.优选订阅生成.local) { // 本地生成订阅
                            ...
                            try {
                                const response = await fetch(优选订阅生成器URL, { headers: { 'User-Agent': 'v2rayN/edge' + 'tunnel (mo' + 'mo)' } });
                        ...
                        try {
                            const response = await fetch(订阅转换URL, { headers: { 'User-Agent': 'Subconverter for ' + 订阅类型 + ' edge' + 'tunnel(mo' + 'mo)' } });
                            if (response.ok) {
...
async function 读取config_JSON(env, hostname, userID, 重置配置 = false) {
    ...
    const 默认配置JSON = {
        ...
        订阅转换配置: {
            ...
            SUBCONFIG: "./ACL4SSR_Online_Mini_MultiMode.ini",
            SUBEMOJI: false,
        },
        反代: {
            PROXYIP: "auto",
...
async function 生成随机IP(request, count = 16, 指定端口 = -1) {
    ...
    const cfname = { '9808': 'Mo移动优选', '4837': 'Mo联通优选', '4134': 'Mo电信优选' }[asn] || 'Mo官方优选';
